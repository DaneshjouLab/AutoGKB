...
class AnnotationPipeline:
    def __init__(self, pmcid: str, citation_model: str = "local", use_one_shot_citations: bool = False):
    ...
    def run(self, save_path: str = "data/one_shot_annotations"):
        ...
        if self.use_one_shot_citations:
            logger.info(f"Adding Citations to Annotations using OneShotCitations with model {self.citation_model}")
            for relationship in self.annotations.relationships:
                citations = self.one_shot_citations.get_annotation_citations(relationship, model=self.citation_model)
                relationship.citations = citations
            
            logger.info(f"Adding Citations to Study Parameters using OneShotCitations with model {self.citation_model}")
            for field_name in self.study_parameters.model_fields:
                if field_name != "additional_resource_links":  # Skip non-ParameterWithCitations field
                    param_content = getattr(self.study_parameters, field_name)
                    if hasattr(param_content, 'content'):
                        citations = self.one_shot_citations.get_study_parameter_citations(field_name, param_content.content, model=self.citation_model)
...
if __name__ == "__main__":
    ...
    for pmcid in pmcids:
        ...
        pipeline = AnnotationPipeline(pmcid, citation_model="openai/gpt-4.1", use_one_shot_citations=True)
        pipeline.run()
    for pmcid in pmcids:
        copy_markdown(pmcid)
