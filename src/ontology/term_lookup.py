...
class TermLookup:
    ...
    def lookup_variant(self, variant: str, threshold: float = 0.8, top_k: int = 1) -> Optional[List[VariantSearchResult]]:
        return self.variant_search.search(variant, threshold=threshold, top_k=top_k)
    
    def lookup_drug(self, drug: str, threshold: float = 0.8, top_k: int = 1) -> Optional[List[DrugSearchResult]]:
        return self.drug_search.search(drug, threshold=threshold, top_k=top_k)
    
    def search(self, term: str, term_type: TermType, threshold: float = 0.8, top_k: int = 1) -> Optional[List[VariantSearchResult]] | Optional[List[DrugSearchResult]]:
